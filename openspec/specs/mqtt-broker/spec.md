...
When SERVER TYPE is `local` and the broker is listening, the host MUST use that local broker for device topic publish and subscribe and MUST NOT use the SERVER field. Device topic publish and subscribe behavior MUST stay the same as remote-client mode.
